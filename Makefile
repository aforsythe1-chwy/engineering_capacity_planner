...
# Override when your persistent planner database lives elsewhere:
...
dev-test-db:
	source /Users/aforsythe1/.nvm/nvm.sh && nvm use && ECP_TEST_DB=true ECP_DB_PATH="$(ECP_DB_PATH)" npm run dev

## Start the planner against the persistent ECP_DB_PATH for durable changes.
dev-persistent-db:
	source /Users/aforsythe1/.nvm/nvm.sh && nvm use && ECP_TEST_DB=false ECP_DB_PATH="$(ECP_DB_PATH)" npm run dev

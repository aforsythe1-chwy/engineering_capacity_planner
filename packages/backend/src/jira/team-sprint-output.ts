...
export async function getTeamSprintOutput(client: JiraClient | undefined, data: DomainDataset, teamId: string, jiraBaseUrl?: string | null): Promise<TeamSprintOutput> {
  ...
  const withCapacities = (sprint: SprintWindow | null): EngineerSprintOutput[] => members.map((member) => {
    const context = sprint ? buildCapacityContext({ members: [member], pto: data.pto.filter((item) => item.memberId === member.id), oncall: data.oncall.filter((item) => item.memberId === member.id), velocityOverrides: data.velocityOverrides.filter((item) => item.memberId === member.id), holidays: (data.holidays ?? []).filter((holiday) => holiday.teamId === teamId), oncallMultiplier: numberSetting(data.settings, SETTING_KEYS.ONCALL_MULTIPLIER, ENGINE_DEFAULTS.ONCALL_MULTIPLIER) }) : null;

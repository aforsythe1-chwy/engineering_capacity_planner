...
export function RunStandupPage({ dataset, projection, teamId, editable, onTeamChange }: { dataset: DomainDataset; projection: PortfolioProjection; teamId: string | null; editable: boolean; onTeamChange: (teamId: string) => void }) {
  ...
  const openRecord = async (sessionId: string) => { try { setOpensWithOpener(false); setAggregate(await api.getStandup(sessionId)); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } };

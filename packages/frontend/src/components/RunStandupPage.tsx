...
import { StandupSprintOpener } from './StandupSprintOpener';
import { StandupRecords } from './StandupRecords';
...
export function RunStandupPage({ dataset, projection, teamId, editable, testMode, onTeamChange }: { dataset: DomainDataset; projection: PortfolioProjection; teamId: string | null; editable: boolean; testMode: boolean; onTeamChange: (teamId: string) => void }) {
  const team = dataset.teams.find((candidate) => candidate.id === teamId) ?? dataset.teams[0] ?? null;
  const [aggregate, setAggregate] = useState<api.StandupAggregate | null>(null); const [error, setError] = useState<string | null>(null); const launch = useRef<HTMLButtonElement>(null); const recordRequest = useRef(0);
  ...
  const [opensWithOpener, setOpensWithOpener] = useState(false);
  const open = async () => { if (!team || !editable) return; setError(null); try { setOpensWithOpener(true); setAggregate(await api.startStandup(team.id, localToday())); await refreshSessions(); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } };
  if (!team) return <main className="team-page"><section className="panel">No team is configured yet.</section></main>;
  const openRecord = async (sessionId: string) => { const request = recordRequest.current + 1; recordRequest.current = request; setOpeningRecordId(sessionId); setError(null); try { const value = await api.getStandup(sessionId); if (recordRequest.current !== request) return; setOpensWithOpener(false); setAggregate(value); } catch (e) { if (recordRequest.current === request) setError(e instanceof Error ? e.message : String(e)); } finally { if (recordRequest.current === request) setOpeningRecordId(null); } };
  const deleteRecord = async (session: import('@ecp/shared').StandupSession) => { if (!window.confirm('Delete this standup and its saved bandwidth and notes?')) return; setDeletingRecordId(session.id); setError(null); try { await api.deleteStandup(session.id); setSessions((current) => current.filter((item) => item.id !== session.id)); void refreshSessions(); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setDeletingRecordId(null); } };
  return <main className="team-page" data-testid="standup-page"><section className="panel team-header standup-launch"><div><h2>Standup</h2><p className="hint">Review the current sprint, capture today’s bandwidth, and record team follow-ups.</p></div><div className="team-controls">{dataset.teams.length > 1 && <label className="control"><span>Team</span><select value={team.id} onChange={(e) => onTeamChange(e.target.value)}>{dataset.teams.map((t) => <option value={t.id} key={t.id}>{t.name}</option>)}</select></label>}<button ref={launch} className="btn primary" disabled={!editable} onClick={() => { void open(); }}>Start Standup</button></div></section><StandupRecords sessions={sessions} includeWeekends={testMode} openingSessionId={openingRecordId} deletingSessionId={deletingRecordId} onOpen={(sessionId) => void openRecord(sessionId)} onDelete={deleteRecord} />{!editable && <div className="panel config-notice">Bundled sample data is read-only. Start the backend to facilitate a standup.</div>}{error && <div className="panel config-error" role="alert">⚠ {error}</div>}{aggregate && <StandupModal dataset={dataset} projection={projection} initialPhase={opensWithOpener ? 'opener' : 'round'} aggregate={aggregate} onChange={(value) => { setAggregate(value); void refreshSessions(); }} onClose={() => { setAggregate(null); void refreshSessions(); launch.current?.focus(); }} onError={setError} />}</main>;
}

function StandupModal({ dataset, projection, initialPhase, aggregate, onChange, onClose, onError }: { dataset: DomainDataset; projection: PortfolioProjection; initialPhase: 'opener' | 'round'; aggregate: api.StandupAggregate; onChange: (value: api.StandupAggregate) => void; onClose: () => void; onError: (value: string | null) => void }) {
  ...
  useStandupWalkOffAudio({ settings: audioSettings, memberId: currentMemberId, heat: timerState.heat, paused: timerState.paused, active: aggregate.session.status === 'active' && phase === 'round' });
  ...
  const isOpener = aggregate.session.status === 'active' && Boolean(current) && phase === 'opener'; const isActiveRound = aggregate.session.status === 'active' && Boolean(current) && phase === 'round';
  const subtitle = isOpener ? `${aggregate.session.sprintName ?? 'Sprint unavailable'} · ${aggregate.session.date}` : `${aggregate.session.sprintName ?? 'Sprint unavailable'} · ${aggregate.session.date} · ${aggregate.session.status === 'active' ? `${done} of ${aggregate.participants.length}` : aggregate.session.status === 'completed' ? 'Completed' : 'Post-standup'}`;
  return <div className="modal-backdrop standup-backdrop" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><section className={`modal standup-modal${isActiveRound ? ' is-active-round' : ''}${timerState.overTime ? ` is-over-time standup-heat-${timerState.heat}` : ''}${timerState.paused ? ' is-timer-paused' : ''}`} role="dialog" aria-modal="true" aria-labelledby="standup-title">{timerState.overTime && <StandupFireEffect paused={timerState.paused} heat={timerState.heat} />}<header className="modal-heading"><div><h2 id="standup-title">{isOpener ? 'Sprint opener' : aggregate.session.status === 'post_standup' ? 'Post-standup notes' : 'Standup'}</h2><p className="hint">{subtitle}</p></div><button className="modal-close" aria-label="Close standup" onClick={onClose}>×</button></header>{isOpener ? <StandupSprintOpener dataset={dataset} projection={projection} session={aggregate.session} /> : isActiveRound && current ? <TeamRound dataset={dataset} aggregate={aggregate} participant={current} ticketContext={ticketStates[current.memberId]?.context ?? null} ticketRefreshing={ticketStates[current.memberId]?.refreshing ?? false} onChange={onChange} onSelectionChange={setCanAdvance} timer={<StandupSpeakerTimer timerKey={`${aggregate.session.id}:${current.memberId}`} thresholdSeconds={threshold} onOvertimeChange={handleOvertimeChange} />} /> : aggregate.session.status === 'post_standup' ? <PostStandup dataset={dataset} aggregate={aggregate} onChange={onChange} intake={intakeState} onSync={() => loadIntake(true)} /> : <Notes dataset={dataset} aggregate={aggregate} onChange={onChange} />}{isOpener && <footer className="modal-actions standup-actions"><button className="btn primary" onClick={() => setPhase('round')}>Begin team round</button></footer>}{isActiveRound && <footer className="modal-actions standup-actions" data-testid="standup-actions"><button className="btn" onClick={() => void resolve('skipped')}>Skip</button><button className="btn primary" disabled={!canAdvance} onClick={() => void resolve('completed')}>Next</button></footer>}{aggregate.session.status === 'post_standup' && <footer className="modal-actions"><button className="btn primary" onClick={() => void finish()}>Finish Standup</button></footer>}{aggregate.session.status === 'completed' && <footer className="modal-actions"><button className="btn danger" onClick={() => void remove()}>Delete</button></footer>}</section></div>;
}
...
function StandupTickets({ context, refreshing, entries }: { context: StandupMemberTicketContext | null; refreshing: boolean; entries?: import('@ecp/shared').StandupStatusPresentationEntry[] }) {
  ...
  return <section className="standup-tickets" aria-live="polite" aria-busy={refreshing}><div className="standup-ticket-heading"><h3>Sprint tickets</h3>{refreshing ? <span className="standup-ticket-loading"><span className="standup-spinner" aria-hidden="true" />Refreshing</span> : <time className="standup-ticket-freshness" dateTime={context.capturedAt} title={refreshedTitle}>Updated {refreshed}</time>}</div>{context.freshness === 'stale' && <p className="hint">Showing the last saved tickets; refresh failed.</p>}{groups.length ? <div className="standup-ticket-groups">{groups.map((group) => {
    ...
    return <section className={`standup-ticket-group tone-${tone}`} key={group.identity} aria-labelledby={headingId}><h4 id={headingId}><span className="standup-ticket-status-marker" aria-hidden="true" /><span>{group.displayName}</span><span className="badge" aria-label={`${group.tickets.length} tickets`}>{group.tickets.length}</span></h4><ul className="standup-ticket-list">{group.tickets.map((ticket) => <li className="standup-ticket" key={ticket.key}>{ticket.url ? <a className="standup-ticket-card" href={ticket.url} target="_blank" rel="noreferrer"><span className="standup-ticket-key">{ticket.key}<span className="standup-ticket-external" aria-hidden="true">↗</span></span><span className="standup-ticket-summary" title={ticket.summary}>{ticket.summary}</span><span className="sr-only">Opens in a new tab</span></a> : <article className="standup-ticket-card"><strong className="standup-ticket-key">{ticket.key}</strong><span className="standup-ticket-summary" title={ticket.summary}>{ticket.summary}</span></article>}</li>)}</ul></section>;

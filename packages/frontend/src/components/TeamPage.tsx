...
export function TeamPage({ dataset, teamId, editable, onReload, onTeamChange }: {
  ...
  const moveAvailability = async (entry: AvailabilityEntry, startDate: string, endDate: string) => {
    try { const remove = entry.kind === 'pto' ? api.deletePto : entry.kind === 'oncall' ? api.deleteOncall : api.deleteVelocityOverride; await remove(entry.id); if (entry.kind === 'pto') await api.createPto({ memberId: entry.memberId, startDate, endDate, note: entry.note }); else if (entry.kind === 'oncall') await api.createOncall({ memberId: entry.memberId, startDate, endDate, note: entry.note }); else await api.createVelocityOverride({ memberId: entry.memberId, startDate, endDate, multiplier: entry.multiplier ?? 1, note: entry.note }); await onReload(); } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); }
  };

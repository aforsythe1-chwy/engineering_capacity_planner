...
export type SprintProgressModel = { state: 'unavailable' | 'empty' | 'ready'; totalItems: number; estimatedItems: number; unestimatedItems: number; totalPoints: number; points: Record<'Done' | 'In Review' | 'In Progress' | 'To Do', number>; donePercent: number | null; reviewPercent: number | null; inProgressPercent: number | null; toDoPercent: number | null; totalDays: number | null; elapsedDays: number | null; elapsedPercent: number | null; signal: string | null; accessibleSummary: string };
...
export function deriveStandupSprintProgress(context: StandupSprintProgressContext | null, sessionDate: string): SprintProgressModel {
  ...
  const donePercent = percent(points.Done, totalPoints); const reviewPercent = percent(points.Done + points['In Review'], totalPoints); const inProgressPercent = percent(points.Done + points['In Review'] + points['In Progress'], totalPoints); const toDoPercent = percent(totalPoints, totalPoints);
  ...
  return { state, totalItems: context.items.length, estimatedItems, unestimatedItems, totalPoints, points, donePercent, reviewPercent, inProgressPercent, toDoPercent, totalDays, elapsedDays, elapsedPercent, signal, accessibleSummary: `${totalPoints} estimated points across ${context.items.length} items. Done ${points.Done} points. Done plus review ${points.Done + points['In Review']} points. Including in progress ${points.Done + points['In Review'] + points['In Progress']} points. Including to do ${totalPoints} points.${elapsedDays !== null && totalDays !== null ? ` Day ${elapsedDays} of ${totalDays}.` : ' Sprint dates unavailable.'}` };
}

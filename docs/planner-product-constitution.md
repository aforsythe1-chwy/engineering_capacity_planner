...
Overview, Timeline, Dependencies, Gantt Planner, and Configuration are peer pages in one planner.
An epic must not become a child application, drill-down mode, or prerequisite navigation layer.

### 2. Epic selection is a filter

Selecting an epic changes the visible scope of the current page. It must not implicitly navigate,
replace the application shell, reveal a separate set of primary pages, or require a special action
...
[`flattened-planner-layout-plan.md`](./flattened-planner-layout-plan.md). Plans may evolve as the
code changes, but the principles above remain the review boundary unless explicitly amended.
